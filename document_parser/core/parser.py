...
class DocumentParser:
    ...
    def render_simple_template_file(
        ...
        if not self._template_processor._env:
            raise DocumentParsingError("No template directory configured for file templates")
        
        try:
            template = self._template_processor._env.get_template(template_filename)
            rendered = template.render(**context)
            logger.info(f"Successfully rendered simple template file: {template_filename}")
            return rendered
        except Exception as e:
            raise DocumentParsingError(f"Error rendering simple template file: {str(e)}")
    ...
    def render_data_file_to_template(
        ...
        try:
            ...
            logger.info(f"Successfully rendered data file to template")
            return rendered
        ...
        try:
            ...
            for template_filename in template_filenames:
                try:
                    ...
                    logger.error(f"Failed to render template {template_filename}: {str(e)}")
