...
@click.option('--output-dir', type=click.Path(), help='Output directory for multiple files')
...
def parse(files: tuple, output: str, output_dir: str, pretty: bool) -> None:
    ...
    try:
        ...
        if len(file_list) > 1 and output:
            click.echo("Error: Cannot use --output with multiple files. Use --output-dir instead.", err=True)
            sys.exit(1)
        ...
        for file_path in file_list:
            try:
                ...
                click.echo(f"Unexpected error processing {file_path}: {str(e)}", err=True)
                continue
...
@click.option('--template-file', type=click.Path(exists=True), help='Template file to use')
@click.option('--template-example', type=click.Choice(['summary', 'detailed_report', 'json_summary']), 
              help='Use a predefined template example')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--template-dir', type=click.Path(), help='Directory containing template files')
def render(file_path: str, template: str, template_file: str, template_example: str, 
           output: str, template_dir: str) -> None:
    """
    ...
    try:
        ...
        if template:
            ...
            if not template_string:
                click.echo(f"Error: Template example '{template_example}' not found", err=True)
                return
        elif template_file:
            ...
            click.echo("Error: Must specify --template, --template-file, or --template-example", err=True)
            return
...
def templates() -> None:
    ...
    click.echo('document-parser render document.pdf --template "{{ document.filename }}: {{ total_pages }} pages"')
