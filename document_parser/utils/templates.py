...
class TemplateProcessor:
    ...
    def _setup_environment(self) -> None:
        ...
        if self.template_dir and os.path.exists(self.template_dir):
            ...
            logger.info(f"Template environment setup with directory: {self.template_dir}")
        else:
    ...
    def render_from_file(
        ...
        try:
            ...
            rendered = template.render(**context)
            logger.info(f"Template rendered successfully from file: {template_filename}")
        ...
        for template_def in templates:
            ...
            try:
                if 'string' in template_def:
                    ...
                    logger.warning(f"Template '{template_name}' missing 'string' or 'file'")
                    continue
    ...
    def _build_context(
        ...
        context = {
            ...
            'pages_dict': [page.to_dict() for page in document_result.pages],  # Dictionary version
            'total_pages': len(document_result.pages),
            'total_words': sum(page.metadata.word_count for page in document_result.pages),
            'total_chars': sum(page.metadata.char_count for page in document_result.pages),
            'all_text': '\n\n'.join(page.content.text for page in document_result.pages),
            'all_paragraphs': [
                para for page in document_result.pages 
                for para in page.content.paragraphs
            ],
            'all_headings': [
                heading for page in document_result.pages 
        ...
        context.update({
            'get_page': lambda n: document_result.pages[n-1] if 1 <= n <= len(document_result.pages) else None,
            'get_headings_by_level': lambda level: [
                h for page in document_result.pages 
                for h in page.content.headings 
                if h.level == level
            ],
            'word_count_range': lambda min_words, max_words: [
                page for page in document_result.pages 
    ...
    def save_rendered_template(
        ...
        Args:
            ...
            extra_context: Additional context variables
        """
        rendered = self.render_from_string(template_string, document_result, extra_context)
