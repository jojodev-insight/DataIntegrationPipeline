...
class CSVParser(BaseParser):
            ...
            for sep in separators:
                try:
                    ...
                    if len(df.columns) > 1 or len(df) > 0:
                        logger.debug(f"Successfully read CSV with encoding={encoding}, separator='{sep}'")
                        return df
    ...
    def _dataframe_to_content(self, df: pd.DataFrame, file_path: str) -> PageContent:
        ...
        if not df.empty:
            # Summary paragraph
            paragraphs.append(f"CSV file with {len(df)} rows and {len(df.columns)} columns")
            ...
            for index, row in df.head(max_rows).iterrows():
                ...
                if any(val.strip() for val in row_values):  # Only non-empty rows
                    ...
                    row_description = ", ".join([f"{k}: {v}" for k, v in row_dict.items() if v.strip()])
        ...
        # Create headings
        headings = [HeadingInfo(level=1, text=f"CSV File: {os.path.basename(file_path)}")]
